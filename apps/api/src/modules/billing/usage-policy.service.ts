...
export class UsagePolicyService {
  ...
  async assertCanCreateActiveTask(
    organizationId: string,
    tx: Prisma.TransactionClient,
  ) {
  ...
  private async assertWithinPlanLimit(
    ...
    const billing = await this.billingService.getByOrganizationIdOrThrow(
      organizationId,
      tx,
    );
    ...
    if (currentUsage >= limit) {
      throw new ForbiddenException(
        `Plan limit reached for ${usageMetricLabels[metric]}`,
      );
    }
  }

  private async acquireOrganizationUsageLock(
    organizationId: string,
    tx: Prisma.TransactionClient,
  ) {
    await tx.$executeRaw`
